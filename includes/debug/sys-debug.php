...
if(!function_exists('_sdbug_change_log_location')) {
	...
	function _sdbug_location_plugins($path) {
		global $_sys_debug;
		if($_sys_debug) $_sys_debug->change_plugins_log_location($path, 100);
	}
	function _sdbug_location_themes($path) {
		global $_sys_debug;
		if($_sys_debug) $_sys_debug->change_themes_log_location($path, 100);
	}
}
