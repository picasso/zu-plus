...
class ZU_Debug extends zuplus_Addon {
	...
	protected function construct_more() {
		...
		$this->use_kint = $this->check_option('use_kint');
		...
		if($this->check_option('debug_bar')) $this->dbug_bar = new ZU_DebugBar($this->profiler, $this->output_html, $this->use_kint);
	}
	...
	public function enqueue_kint_css($plugin) {

		if($this->use_kint) {
			$plugin_file = $plugin->config['plugin_file'];
			$plugin_version = $plugin->config['version'];

			$filename = 'css/kint.css';
			$filepath = plugin_dir_path($plugin_file).$filename;
			if(file_exists($filepath)) {
				$version = filemtime($filepath);
				wp_enqueue_style('kint-style', plugins_url($filename, $plugin_file), [], defined('ZUDEBUG') ? $version : $plugin_version);
			}
		}
	}
