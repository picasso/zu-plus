...
;(function ($) {
		...
		$('.kint-rich dt').each(function () {
			...
			if ($dt.text().indexOf('ZU_LOG:') !== -1) {
				...
				let msg = $dt
					...
					.replace(/\s*\(\d+\)\s*/g, '')
					.replace(/"/g, '')
					.replace('ZU_LOG:', '<span>ZU_LOG:</span>')
				let matches = /\[\s*([^\]]+)]/i.exec(msg)
				if (matches && matches.length > 1)
					msg = msg.replace(matches[0], '[ <b>' + matches[1] + '</b> ]')
				$dt.html(msg)
			}
		})
	}
	...
	function zuplus_click_autosave_notice() {
		...
		$('.components-notice.is-dismissible').each(function () {
			$(this).each(function () {
				...
				forEach(messages, function (value) {
					if (includes(text, value)) {
	...
	function zuplus_bind_dismiss_links() {
		$('.notice.is-dismissible .notice-dismiss').each(function () {
			...
			$link.click(function () {
				...
				zuplus_turn_option('zuplus_dismiss_error', prefix_name)
			})
			if ($link.hasClass('ajax-dismiss'))
				$link.click(function () {
					setTimeout(function () {
						$link.parent().remove()
					}, 100)
	...
	function zuplus_update_UI(option_name, response, $container) {
		...
		switch (option_name) {
			...
			case 'zuplus_clear_errors':
				$('#zuplus-errors-mb .inside')
					.empty()
					.append('<div class="form_desc">There\'re no errors.</div>')
				break
	...
	function zuplus_turn_option(option_name, prefix_name, ajax_value) {
		...
		var $container = $rel.length
			? $rel.parents('.postbox')
			: $('#' + prefix_name + '-options-mb')

		if (data.option_name === 'zuplus_revoke_cookie') {
			document.cookie =
				data.ajax_value +
				'=;Max-Age=-99999999;expires=Thu, 01 Jan 1970 00:00:01 GMT;path=/;'
		}
		...
		$.ajax({
			...
			data,
			success(response) {
				zuplus_update_UI(option_name, response, $container)
			},
			error() {
				$container.removeClass('now_ajaxed')
			},
		})
	}
	...
	$(document).ready(function () {
		if ($('body').hasClass('zuplus')) {
			// only for plugin screen  if($('body').hasClass(zuplus_data.screen_id)) {
			// Bind action links
			zuplus_bind_links()
