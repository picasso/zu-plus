...
const ZuplusCoreInfo = ({ labels, ajaxAction, noticeOperations }) => {
		...
		(uri, id, version, linkedRef) => {
			...
			fetch(url)
				...
				.catch((error) => {
					...
					createNotice({
						status: 'error',
						content: simpleMarkdown(`${labels.error} [*${request}*]: **${error}**`, {
							raw: true,
						}),
						isDismissible: true,
						__unstableHTML: true,
	...
	const updateInstClassName = useCallback(
		(core, current, ref) => {
	...
	const onDynamicCell = useCallback(
		(params) => {
				...
				<div className={cprefix}>
					<ul className="__info">
						...
						<li>
							...
							<span className={mergeClasses('__ver', activeVersion)}>
								{coreData.version}
							</span>
						</li>
						<li>
